...
gaussDriveNoiseSD <- 0.055	#in meter/sec
...
## times for dialing
singleTaskKeyPressTimes <- c(264,264,264,264,264,264,264,264,264,264,264)   #digit times needed per keypress at that specific position (note: normalized for chunk retrieval time at digits 1 and 6 --- a retrieval cost would come on top of this)
...
runAllSimpleStrategies <- function(nrSimulations,phoneNumber)
{
	...
	for (binaryStrategy in 0:1024)
	{
	  strategy <- c()
	  for (digit in 1:phoneStringLength)
	  {
	    #print(intToBits(binaryStrategy)[digit])
	    if (intToBits(binaryStrategy)[digit] == 01)
	    {
	      strategy <-append(strategy,digit)
	      #print(strategy)
	    }
	  }
		...
		for (steerTimes in locSteerTimeOptions)
		{
			for (i in 1:nrSimulations)
			{
			  ...
			  #print(strategy)
				...
				strats <- c(strats,rep(c(toString(strategy)),nrow(locTab)))
...
result <- runAllSimpleStrategies(1, "06239055021")
result1 <- result[result$strats == 5,]
result2 <- result[result$strats != 5,]
with(result2,plot(TrialTime/1000,abs(dev),pch=21,bg="dark grey",col="dark grey",xlab="Dial time (s)",ylab="Average Lateral Deviation (m)", log="x"))
with(result1,lines(type = 'p',TrialTime/1000,abs(dev),pch=21,bg="red",col="red",xlab="Dial time (s)",ylab="Average Lateral Deviation (m)"))
